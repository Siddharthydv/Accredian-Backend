...
router.post("/signin/guest", async (req: Request, res: Response): Promise<void> => {
  try {

    // Find user
    const user = await prismaClient.user.findUnique({ where: { email:'guest@gmail.com' } ,include: {referrals: true}}); // Include all referrals associated with each user
